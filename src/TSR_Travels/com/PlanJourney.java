...
public class PlanJourney {
	...
	private static void displayAllStations() {
		Adminstrator.getAdminObj().getBusList().forEach(bus -> bus.getStationMap().forEach((k, v) -> locations.add(k)));
	...
	private static List<Bus> getAvailableBuses(String boardingStation, String destinationStation) {
		...
		return Adminstrator.getAdminObj().getBusList().stream()
				.filter(bus -> (bus.getStationMap().containsKey(boardingStation)
						&& bus.getStationMap().containsKey(destinationStation))
	...
	protected static double getPrice(Bus bus, String boardingStation, String destinationStation) {
		if (bus.getCategory().equalsIgnoreCase("AC")) {
			if (bus.getBusType().equalsIgnoreCase("Seater"))
				return (bus.getStationMap().get(destinationStation) - bus.getStationMap().get(boardingStation)) * 3.5;
			else
				return (bus.getStationMap().get(destinationStation) - bus.getStationMap().get(boardingStation)) * 5.5;
		} else {
			if (bus.getBusType().equalsIgnoreCase("Seater"))
				return (bus.getStationMap().get(destinationStation) - bus.getStationMap().get(boardingStation)) * 1.5;
			else
				return (bus.getStationMap().get(destinationStation) - bus.getStationMap().get(boardingStation)) * 2.5;
		}
	}
